...
const CupPageDetails = () => {
    ...
    useEffect(() => {
        const fetchCupDetails = async () => {
            try {
                ...
                setSelectedYear(latestSeason.year);// when the page loads year dropdown will set to latest season
                fetchMatchDetails(latestSeason.year); //fetch the latest match details means the last match year when page loads
                setLoading(false);
            } catch (error) {
                setError(error);
                setLoading(false);
    ...
    const fetchMatchDetails = async (season) => {
        try {
            ...
            //if the fixtures exits a clib 

            if (fixtures.length > 0) {
                const finalMatch = fixtures.find(match => match.league.round.includes('Final'));
                const firstScheduledMatch = fixtures.find(match => new Date(match.fixture.date) >= new Date());
                setMatchDetails(finalMatch || firstScheduledMatch || fixtures[0]);
    ...
    const fetchRoundDetails = async (season, round) => {
        try {
            const res = await axios.get(`https://api-football-v1.p.rapidapi.com/v3/fixtures`, {
                headers: {
                    'x-rapidapi-host': 'api-football-v1.p.rapidapi.com',
    ...
    const handleSelectRound = (round) => {
        ...
        fetchRoundDetails(selectedYear, round);
    };
    ...
    const convertToIST = (date) => {
        const options = { year: 'numeric', month: 'long', day: 'numeric', hour: 'numeric', minute: 'numeric', timeZone: 'Asia/Kolkata', timeZoneName: 'short' };
                ...
                <div className="col-span-2 sm:col-span-1 sm:mb-4 lg:col-span-2 mr-3 ml-3">
                    <div className="border rounded-lg bg-gradient-to-r from-slate-100 to-slate-300 bg-opacity-75 p-4 flex items-center">
                        <div className="flex-none mr-4 lg:bg-white">
                            <
                                img src={cupData.league.logo}
                                alt="Club"
                                className="w-16 h-16 lg
                                                                    lg
                                                                    bg-repeat-space"
                            />
                        </div>
                ...
                {matchDetails && (
                    <div className="col-span-1 sm:col-span-1 lg:col-span-1 mr-3 ml-3 mt-5 md:mt-5 lg:mt-0 bg-gradient-to-r from-rose-100 to-zinc-100 border rounded-lg p-4 shadow-md">
                        ...
                        <div className="flex flex-col items-center gap-4">
                            ...
                            <div className="text-gray-500 font-sans text-xl font-bold">Status: {matchDetails.fixture.status.short}</div>
                        </div>
                ...
                <div className="fixed inset-0 bg-blue-300 bg-opacity-75 flex items-center justify-center z-50">
                    <div className="bg-white rounded-lg p-6 w-3/4 md:w-1/2">
                        ...
                        <div className="flex flex-wrap justify-center mb-4 space-x-4">
                            ...
                            <div className={`cursor-pointer border-b-4 lg p-3 transition duration-300 ${selectedScore === 'penalty' ? 'border-blue-500' : ''}`} onClick

                                ={() => handleSelectScore('penalty')}>
                                <h2>Penalty</h2>
                            </div>
